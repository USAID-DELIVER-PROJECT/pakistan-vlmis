...
$(window).load(function() {
  ...
  map = new OpenLayers.Map('map', {
    ...
    district = new OpenLayers.Layer.Vector(
        ...
        vLMIS = new OpenLayers.Layer.Vector("Expiry Alert", {
...
function getData() {
    ...
    function callback(response) {
        ...
        for (var i = 0; i < data.length; i++) {
            chkeArray(data[i].district_id, data[i].quantity, data[i].QuantityIn6Months, Number(data[i].ExpiringIn6Months), Number(data[i].ExpiringIn12Months), Number(data[i].ExpiringIn13Months));
        }
        drawGrid();
        districtCountGraph();
        $("#submit").attr("disabled", false);
...
function chkeArray(district_id, quantity, quantity6Months, sixBelow, sixThirteen, thirteenAbove) {
    for (var i = 0; i < district.features.length; i++) {
        if (district_id == district.features[i].attributes.district_id) {
            vLMISLayer(district.features[i].geometry, district.features[i].attributes.province_name, product_name, district_id, district.features[i].attributes.district_name, quantity, quantity6Months, sixBelow, sixThirteen, thirteenAbove);
            break;
        }
    }
}
...
function vLMISLayer(wkt, province, product, district_id, district_name, quantity, quantity6Months, sixBelow, sixThirteen, thirteenAbove) {
    feature = new OpenLayers.Feature.Vector(wkt);

    if (sixBelow == classesArray[0].start_value && sixBelow == classesArray[0].end_value) {
        color = classesArray[0].color_code;
        NoData = Number(NoData) + 1;
        status = classesArray[0].description;
    }
    if (sixBelow > classesArray[1].start_value && sixBelow <= classesArray[1].end_value) {
        color = classesArray[1].color_code;
        class1 = Number(class1) + 1;
        status = classesArray[1].description;
    }
    if (sixBelow > classesArray[2].start_value && sixBelow <= classesArray[2].end_value) {
        color = classesArray[2].color_code;
        class2 = Number(class2) + 1;
        status = classesArray[2].description;
    }
    if (sixBelow > classesArray[3].start_value && sixBelow <= classesArray[3].end_value) {
        color = classesArray[3].color_code;
        class3 = Number(class3) + 1;
        status = classesArray[3].description;
    }
    if (sixBelow > classesArray[4].start_value && sixBelow <= classesArray[4].end_value) {
        color = classesArray[4].color_code;
        class4 = Number(class4) + 1;
        status = classesArray[4].description;
    }
    if (sixBelow > classesArray[5].start_value) {
    ...
    feature.attributes = {
        ...
        color: color,
        status : status ,
        sixThirteen: sixThirteen,
        thirteenAbove: thirteenAbove
    };
...
function onFeatureSelect(e)
{
    ...
    $("#expiry_rate").html(e.feature.attributes['sixBelow']);
...
function mapTitle() {
    ...
    $("#mapTitle").html("<font color='green' size='4'><b>Stock Expiry in Six Months (" + month_year + ")</b></font> <br/> " + product_name);
...
function SortByID(x, y) {
    return x.ExpiringIn6Months - y.ExpiringIn6Months;
}
...
function drawGrid(){
    ...
    for (var i = 0; i < features.length; i++) {
        table += "<tr><td>" + features[i].attributes.province + "</td><td>" + features[i].attributes.district + "</td><td align='right'>" + features[i].attributes.quantity + "</td><td align='right'>" + features[i].attributes.quantity6Months + "</td><td align='right'>" + features[i].attributes.sixBelow + "</td><td><div style='width:30px;height:18px;background-color:" + features[i].attributes.color + "'></div></td></tr>";
        jsonData.push({
            label: features[i].attributes.district,
            value: features[i].attributes.sixBelow,
        ...
        dataDownload.push({
            ...
            Percent_Expire_6Months: features[i].attributes.sixBelow
        });
    }
    table += "</table>";
