...
class Model_Placements extends Model_Base {
    ...
    public function add() {
        ...
        $placement->setVvmStage($data['vvmstage']);
    ...
    public function addPlacement() {
        ...
        $placement->setVvmStage($form_values['vvm_stage']);
    ...
    public function addTransferStock() {
        ...
        $quantity_per_pack = $form_values['quantity_per_pack'];
        ...
        $placement->setPlacementLocation($placement_location_id);
        $placement->setQuantity($form_values['quantity'] * $quantity_per_pack);

        $placement_id = $form_values['id'];
        $plac = $this->_em->find("Placements", $placement_id);
        $stock_batch = $plac->getStockBatch();
        $stock_detail = $plac->getStockDetail();
        ...
        //  $placement_location_id = $this->_em->getRepository("PlacementLocations")->findOneBy(array("locationId" => $form_values['non_ccm_location_id'], "locationType" => Model_PlacementLocations::LOCATIONTYPE_NONCCM));
        // $placement2->setPlacementLocation($placement_location_id);
        $quantity_per_pack = $form_values['quantity_per_pack'];
        $placement2->setQuantity("-" . $form_values['quantity'] * $quantity_per_pack);

        $placement_id = $form_values['id'];
        $plac = $this->_em->find("Placements", $placement_id);
        $plac_location = $plac->getPlacementLocation();
        $placement2->setPlacementLocation($plac_location);
        $stock_batch = $plac->getStockBatch();
        $stock_detail = $plac->getStockDetail();
        $placement2->setStockBatch($stock_batch);
        $placement2->setStockDetail($stock_detail);
        $plc_transaction_type = $this->_em->find("ListDetail", Model_PlacementLocations::PLACEMENT_TRANSACTION_TYPE_T);
    ...
    public function getStockInBin($order, $sort) {
        ...
        WHERE placements.placement_location_id =" . $this->form_values['id'] . " AND non_ccm_locations.warehouse_id =" . $warehouse_id . " GROUP BY
placements.stock_batch_id";

        if ($order == 'product') {
            $str_sql .= " ORDER BY item_pack_sizes.item_name $sort";
        }
        if ($order == 'batch_no') {
            $str_sql .= " ORDER BY stock_batch.number $sort";
        }
        if ($order == 'carton_qty') {
            $str_sql .= " ORDER BY placements.quantity $sort";
        }
        if ($order == 'qty') {
            $str_sql .= " ORDER BY placements.quantity $sort";
        }
    ...
    public function getStockInBinVaccines($order, $sort) {
        ...
        $str_sql = $this->_em->createQueryBuilder()
                ->select("ps.pkId,SUM(ps.quantity) AS qty,
                            ps.vvmStage,
                            sb.number, DATE_FORMAT(sb.expiryDate,'%d/%m/%Y') AS expiry,
                            sb.pkId as batch_id,
                            ips.itemName")
                ->from("Placements", "ps")
                ...
                ->groupBy("ps.vvmStage,ips.pkId,sb.pkId");

        switch ($order) {
            case 'product':
                $str_sql->orderBy("ips.itemName", $sort);
                break;
            case 'batch_no':
                $str_sql->orderBy("sb.number", $sort);
                break;
            case 'qty':
            ...
            case 'vvm_stage':
                ...
                break;
            default:
                $str_sql->orderBy("ips.itemName,sb.number", "ASC");
                break;
        }

        $str_sql->having("qty > 0");
        //echo $str_sql->getQuery()->getSql();
        //exit;
