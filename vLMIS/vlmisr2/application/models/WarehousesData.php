 ...
 * @version    2
 */
class Model_WarehousesData extends Model_Base {
    ...
    public function getWastages() {
        ...
        /* $where = "";
          ...
          $where = " AND warehouses_data.item_pack_size_id = " . $this->form_values['item'] . "";
          }

          $str_sql = "SELECT
          B.province_id,
          B.location_name AS provinceName,
          COALESCE(reported, NULL, 0) AS reported,
          TotalWH,
          COALESCE(ROUND(((reported / TotalWH) * 100), 1), NULL, 0) AS RptPer,
          COALESCE(wastagePer, NULL, 0) AS wastagePer
          FROM
          (
          SELECT
          UC.province_id,
          ROUND(IFNULL((sum(warehouses_data.wastages) / (SUM(warehouses_data.issue_balance) + sum(warehouses_data.wastages))) * 100,0),1) AS wastagePer,
          ...
          GROUP BY
          warehouses.province_id
          ) AS B ON A.province_id = B.province_id
          GROUP BY
          B.province_id"; */
                    ...
                    FROM
                            (
                                    SELECT
                                            ...
                                            ROUND(
                                                    IFNULL((
                                                                    sum(warehouses_data.wastages) / (SUM(warehouses_data.issue_balance) + sum(warehouses_data.wastages))) * 100,0),1) AS wastagePer,
                                                                    COUNT(DISTINCT UC.pk_id) AS reported
                                    FROM
                                            locations AS UC
                                    INNER JOIN warehouses ON UC.pk_id = warehouses.location_id
...
warehouses.stakeholder_id = 1
    ...
    public function getExpirySchedule() {
        ...
        switch ($level) {
            case 1:
                ...
                break;
            case 2:
                $join = " INNER JOIN warehouses ON stock_batch.warehouse_id = warehouses.pk_id ";
        ...
        $str_sql = "SELECT
			...
			FROM (SELECT
				...
				SUM(IF (stock_batch.expiry_date <= ADDDATE(CURDATE(), INTERVAL 6 MONTH), stock_batch.quantity, 0)) AS Expire6Months,
				SUM(IF (stock_batch.expiry_date > ADDDATE(CURDATE(), INTERVAL 6 MONTH) AND stock_batch.expiry_date <= ADDDATE(CURDATE(), INTERVAL 12 MONTH), stock_batch.quantity, 0)) AS Expire12Months,
				SUM(IF (stock_batch.expiry_date > ADDDATE(CURDATE(), INTERVAL 12 MONTH) AND stock_batch.expiry_date <= ADDDATE(CURDATE(), INTERVAL 18 MONTH), stock_batch.quantity, 0)) AS Expire18Months,
				SUM(IF (stock_batch.expiry_date > ADDDATE(CURDATE(), INTERVAL 18 MONTH), stock_batch.quantity, 0)) AS Expire18Greater
			FROM
				stock_batch
			INNER JOIN item_pack_sizes ON stock_batch.item_pack_size_id = item_pack_sizes.pk_id
                        $join
    ...
    public function getReceiveWarehouses() {
        ...
        $str_sql = "SELECT
                    ...
                    AND stock_batch.item_pack_size_id = $item
                    GROUP BY
                            stock_batch.item_pack_size_id,
                            warehouses.pk_id
                    ORDER BY
...
LEFT JOIN (
	...
	WHERE
        ...
        warehouses.status = 1 AND
        warehouses_data.reporting_start_date = '" . $this->form_values['date'] . "-01'
...
LIMIT 20";
    ...
    public function getSOH() {
        ...
        $str_sql = "SELECT
                    SUM(warehouses_data.closing_balance) AS SOH,
                    warehouses.province_id,
                    locations.location_name
                    FROM
                    warehouses
                    ...
                    WHERE
                    warehouses_data.reporting_start_date = '" . $this->form_values['date'] . "-01' $where
                    GROUP BY
                    warehouses_data.item_pack_size_id,
                    warehouses.province_id";
        $row = $this->_em->getConnection()->prepare($str_sql);
        ...
        $item_pack_sizes->form_values['pk_id'] = $this->form_values['item'];
        $item = $item_pack_sizes->getProductName();
//$xmlstore = '<chart legendPosition="RIGHT" showLegend="1" caption="Provincial SOH" numberPrefix="$" bgColor="FFFFFF" showAlternateHGridColor="0" showPlotBorder="0" divLineColor="CCCCCC" showCanvasBorder="0" canvasBorderColor="CCCCCC" canvasBorderThickness="1" captionPadding="30" use3dlighting="0" showShadow="0">';
...
subCaption = '$item  " . ("(" . date('M Y', strtotime($this->form_values['date'])) . ")" ) . "' exportFileName='Provincial SOH " . date('Y-m-d H:i:s') . "' yAxisName='Stock' showValues='1' formatNumberScale='0'>";
    ...
    public function getSOHByDistricts($prov_id) {
        ...
        $str_sql = "SELECT
            ...
            WHERE
            warehouses_data.reporting_start_date = '" . $this->form_values['date'] . "-01' AND
            warehouses_data.item_pack_size_id = " . $this->form_values['item'] . " AND
...
//$xmlstore = '<chart legendPosition="RIGHT" showLegend="1" caption="Provincial SOH" numberPrefix="$" bgColor="FFFFFF" showAlternateHGridColor="0" showPlotBorder="0" divLineColor="CCCCCC" showCanvasBorder="0" canvasBorderColor="CCCCCC" canvasBorderThickness="1" captionPadding="30" use3dlighting="0" showShadow="0">';
    ...
    public function stockStatusRoutine() {
        ...
        $str_sql = "SELECT
                    ...
                    FROM (SELECT
                            SUM(warehouses_data.issue_balance) AS CONS,
...
WHERE $where ORDER BY wastagePer DESC";
    ...
    public function getMOSDistricts() {
        ...
        $str_sql = "SELECT
                ...
                INNER JOIN locations ON warehouses.province_id = locations.pk_id
                WHERE
                warehouses_data.reporting_start_date = '" . $this->form_values['date'] . "-01' AND warehouses.status = 1 $where
                GROUP BY
                warehouses.province_id) A";
        $row = $this->_em->getConnection()->prepare($str_sql);
        $row->execute();
...
//echo $str_sql;
        ...
        $locations->form_values['pk_id'] = $prov_id;
        $province_name = $this->view->location_name = $locations->getLocationName();
        $xmlstore = "<chart labelDisplay='rotate' exportEnabled='1' exportAction='Download' bgColor='white' caption='Consumption status of $item(Doses) in $province_name Stores  and EPI centers  during " . ("(" . date('M Y', strtotime($this->form_values['date'])) . ")" ) . "'
 ...
 exportFileName='Districts Consumption " . date('Y-m-d H:i:s') . "' yAxisName='Doses' showValues='1' formatNumberScale='0' theme='fint'>";
    ...
    /* public function getConsumptionByUc($dist_id) {
      ...
      locations.location_name as ucName
      FROM
      locations where locations.geo_level_id=6 AND locations.parent_id = $teh_id";
      $row = $this->_em->getConnection()->prepare($str_sql);
      $row->execute();
      $result = $row->fetchAll();
      $item_pack_sizes = new Model_ItemPackSizes();
      ...
      $xmlstore = "<chart exportEnabled='1' exportAction='Download' bgColor='white' caption='Consumption status of $item(Doses) in $province_name UCs  during " . ("(" . date('M Y', strtotime($this->form_values['date'])) . ")" ) . "'
      exportFileName='Districts Doses " . date('Y-m-d H:i:s') . "' yAxisName='Doses' showValues='1' formatNumberScale='0' theme='fint'>";
      foreach ($result as $data) {
      $xmlstore .= "<set label='$data[ucName]' value='$data[CONS]' />";
      }
      $xmlstore .="</chart>";
    ...
    public function stockPosition() {
        $str_sql = "SELECT
                ...
                FROM (SELECT
                        item_pack_sizes.item_name,
                        Sum(warehouses_data.issue_balance) AS CONS,
                        REPgetConsumptionAVG('N', 1, 2014, warehouses_data.item_pack_size_id, 1, 0, 0) AS AvgCONS,
                        Sum(warehouses_data.closing_balance) AS SOH
                FROM
                warehouses
                INNER JOIN stakeholders ON warehouses.stakeholder_office_id = stakeholders.pk_id
                ...
                AND warehouses.status = 1
                warehouses_data.reporting_start_date = '2014-01-01' AND
                warehouses.province_id = 1 AND
                item_pack_sizes.item_category_id = 1
                GROUP BY
                warehouses_data.item_pack_size_id) A";
        $row = $this->_em->getConnection()->prepare($str_sql);
        $row->execute();
        $result = $row->fetchAll();
    ...
    public function create() {
        $warehouse_data = new WarehousesData();
        ...
        $warehouse_data->setCreatedBy($this->form_values['created_by']);
        $warehouse_data->setCreatedDate(new \DateTime(App_Controller_Functions::dateToDbFormat(date("d/m/Y"))));
        $warehouse_data->setModifiedDate(new \DateTime(App_Controller_Functions::dateToDbFormat(date("d/m/Y"))));
    ...
    public function update() {
        $warehouse_data = $this->_em->getRepository("WarehousesData")->find($this->form_values['pk_id']);
        $warehouse_data->setReceivedBalance($this->form_values['received_balance']);
        $warehouse_data->setClosingBalance($this->form_values['closing_balance']);
    ...
    /* public function getMonthYearByWarehouseId() {
      ...
      ->select('
      wd.warehouse_id,
      MONTH(wd.reporting_start_date) as report_month,
      YEAR(wd.reporting_start_date) as report_year,
      wh.location_id
      ')
      ->from("Model_WarehousesData wd")
      ->innerJoin("wd.Warehouses wh")
      ->where("wd.warehouse_id = ? ", $this->warehouse_id)
      ->orderBy("wd.reporting_start_date DESC");
      echo $str_sql->getSqlQuery();die();
      $rs = $str_sql->fetchArray();
      if (!empty($rs) && count($rs) > 0) {
      return $rs;
      } else {
      return false;
...
//App_Controller_Functions::pr($array_data);
        return $array_data;
    }
    ...
    public function addMonthlyConsumption() {
        ...
        if ($data['is_new_report'] == 0) {
            $rows = $this->_table->findBy(array('warehouse' => $data['wh_id'], 'reportingStartDate' => $data['rpt_date']));
            if (count($rows) > 0) {
        ...
        foreach ($closing_balance_array as $key => $val) {
            if ($val != '' && $val >= 0) {
                ...
                $wh_data->setCreatedBy($this->_user_id);
                ...
                $wh_data->setIsCalculated(0);

                $this->_em->persist($wh_data);
                $this->_em->flush();

                // Deleted data from Draft
                $rows = $this->_em->getRepository('WarehousesDataDraft')->findBy(array('warehouse' => $data['wh_id'], 'reportingStartDate' => $data['rpt_date']));
                if (count($rows) > 0) {
                    foreach ($rows as $row) {
                        $this->_em->remove($row);
                    }
    ...
    public function addMonthlyConsumptionDraft() {
        ...
        foreach ($closing_balance_array as $key => $val) {
            if ($val != '' && $val >= 0) {
                ...
                $wh_data = new WarehousesDataDraft();
                $item = $this->_em->getRepository('ItemPackSizes')->find($posted_array[$key]);
                $warehouse = $this->_em->getRepository('Warehouses')->find($data['wh_id']);
                $wh_data->setItemPackSize($item);
                $wh_data->setWarehouse($warehouse);
                ...
                $wh_data->setWastages($wastages);
                $wh_data->setModifiedDate(new \DateTime(App_Controller_Functions::dateToDbFormat($date)));
...
//                        $date = $date->format("Y-m-d");
//                        $this->_em = Zend_Registry::get('doctrine');
//                        $row_12 = $this->_em->getConnection()->prepare("SELECT REPUpdateCarryForward2('" . $date . "'," . $itemid . "," . $wh_id . ")  from DUAL");
//                        $row_12->execute();
//                    }
//                }
                /* ----------------- End ----------------- */
...
//$reports = new Model_Reports();
    ...
    public function getWarehouseDataByMonthAndYearForNational($q_str_pro = "", $q_str_stk = "") {
        $str_sql = "SELECT DISTINCT warehouses.pk_id, warehouses.warehouse_name, warehouses.stakeholder_id,
            ...
            AND warehouses.status = 1
            AND MONTH(warehouses_data.reporting_start_date)= '" . $this->temp['month_sel'] . "'
            AND YEAR(warehouses_data.reporting_start_date)= '" . $this->temp['year_sel'] . "'
            $q_str_pro $q_str_stk";
    ...
    public function getStockAnalysisDistrictWiseReport() {
        ...
        $str_sql = "SELECT
                ...
                FROM
                item_pack_sizes
                INNER JOIN stock_batch ON item_pack_sizes.pk_id = stock_batch.item_pack_size_id
                INNER JOIN stock_detail ON stock_batch.pk_id = stock_detail.stock_batch_id
                INNER JOIN stock_master ON stock_detail.stock_master_id = stock_master.pk_id
