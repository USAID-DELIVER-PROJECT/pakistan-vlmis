...
//->setParameter("array_ids", $ids);
    ...
    public function getTempStocksList() {
        $str_sql = $this->_em->createQueryBuilder()
                ...
                ->innerJoin("sm.fromWarehouse", "fw")
                ->innerJoin("sm.toWarehouse", "tw")
                ->innerJoin("sm.stakeholderActivity", "a")
    ...
    public function getTempStockIssue() {
        ...
        $str_sql = $this->_em->createQueryBuilder()
                ->select("s.transactionDate,"
                        ...
                        . "p.itemName, "
                        . "p.numberOfDoses as description, sd.vvmStage")
                ->from("StockDetail", "sd")
                ->join("sd.stockMaster", "s")
                ->join("sd.stockBatch", "b")
                ->join("b.warehouse", "w")
...
//echo $str_sql->getQuery()->getSql();
    ...
    public function stockIssueSearch() {
        ...
        $str_sql = $this->_em->createQueryBuilder()->select("s.transactionDate,"
                        . " s.pkId,s.transactionNumber,"
                        . "s.transactionReference,"
...
//  exit;
    ...
    public function stockAdjustment() {
        ...
        if (!empty($this->form_values['date_from']) && !empty($this->form_values['date_to'])) {
            $where[] = "s.transactionDate BETWEEN '" . $this->form_values['date_from'] . "' AND '" . $this->form_values['date_to'] . "'";
        }
        $where[] = "s.transactionType > 2";
        if (is_array($where)) {
            $where_s = implode(" AND ", $where);
    ...
    public function expiredStockReport() {
        ...
        $str_sql = "SELECT
	i4_.item_name,
	ABS(SUM(s2_.quantity)) quantity,
                DATE_FORMAT(s1_.expiry_date,'%m/%Y') expiry_date,
        ...
        DATE_FORMAT(s0_.transaction_date,'%Y-%m-%d') BETWEEN '" . $this->form_values['date_from'] . "' AND '" . $this->form_values['date_to'] . "'
        GROUP BY
                s1_.item_pack_size_id,DATE_FORMAT(s1_.expiry_date,'%m/%Y')
        ORDER BY
                transaction_types.transaction_type_name ASC,
                i4_.list_rank ASC";
    ...
    public function getStocksReceiveList($type = 1) {
        ...
        /* $str_sql = $this->_em->createQueryBuilder()
          ...
          . "p.numberOfDoses AS description,"
          . "v.vvmTypeName,"
          . "smp.parentId AS stockMasterParentId")
          ->from("StockDetail", "sd")
          ->join("sd.stockMaster", "s")
          ->join("sd.stockBatch", "b")
          ->join("b.warehouse", "w")
          ...
          ->join("p.itemUnit", "i")
          ->join("b.vvmType", "v")
          ->leftJoin("s.parentId", "smp")
          ->where("s.transactionType = 1")
          ->andWhere("s.toWarehouse = " . $this->_identity->getWarehouseId())
          ->andWhere("s.createdBy = " . $this->_user_id)
...
//$row2->execute();
    ...
    public function addAjustment() {
        ...
        if ($stock_batch->getQuantity() == 0 && $type_nature[0]['nature'] == '-') {
            return false;
            exit;
        }
        ...
        $fy_dates = App_Controller_Functions::getFiscalYear();

        $last_id = $this->getAdjLastID($fy_dates['from_date'], $fy_dates['to_date']);


        if ($last_id['0']['Maxtr'] == NULL) {
            $last_id['0']['Maxtr'] = 0;
        }

        $trans_no = "A" . substr($yy, -2) . $mm . str_pad(($last_id['0']['Maxtr'] + 1), 4, "0", STR_PAD_LEFT);
        $stock_master->setTransactionNumber($trans_no);
//$stock_master->BatchID = $batch_id;
        $stock_master->setDraft(0);
        $stock_master->setTransactionCounter($last_id['0']['Maxtr'] + 1);
        $stock_master->setParentId(0);
        $this->_em->persist($stock_master);
        $this->_em->flush();
        ...
        if ($str_do == "Add") {
            ...
            $stock_detail->setVvmStage($this->form_values['vvm_stage']);
        ...
        if (!empty($this->form_values['location_id'])) {
            $placements = new Model_Placements();
            $placements->form_values = array(
                'batch_id' => $batch_id,
                'placement_loc_id' => $this->form_values['location_id'],
            ...
            /* $str_sql = $this->_em->createQueryBuilder()
              ->select("MAX(ps.vvmStage) as vvm_stage")
              ->from("PlacementSummary", "ps")
              ->where("ps.placementLocation = " . $this->form_values['location_id'])
              ->andWhere("ps.stockBatch = " . $batch_id);
              $row = $str_sql->getQuery()->getResult();
              $old_vvm = '';
              if (count($row) > 0) {
              $old_vvm = $row[0]['vvm_stage'];
              } */

            $old_vvm = $this->form_values['old_vvm'];
    ...
    public function getwarehouseStockByIssueNo() {
        $str_sql = $this->_em->createQueryBuilder()
                ->select("s.pkId,"
                        ...
                        . "sd.pkId as detailId,"
                        . "b.number,"
                        . "p.itemName,DATE_FORMAT(s.transactionDate,'%d/%m/%Y') as transactionDate")
                ->from("StockDetail", "sd")
                ->join("sd.stockMaster", "s")
                ->join("sd.stockBatch", "b")
                ->join("b.itemPackSize", "p")
    ...
    public function addStockWarehouseByIssue() {
        ...
        if (isset($this->form_values['stock_id']) && !empty($this->form_values['stock_id'])) {
            ...
            if (count($stock_detail_result) > 0) {
                ...
                $stock_master->setTransactionNumber($trans_no['trans_no']);
                //$stakeholder_activity = $this->_em->getRepository('StakeholderActivities')->find($this->form_values['stk_activity']);
        ...
        if (isset($this->form_values['stockid']) && !empty($this->form_values['stockid'])) {
            ...
            foreach ($stock_ids as $index => $detail_id) {
                ...
                if (isset($array_missing[$index]) && !empty($array_missing[$index])) {
                    ...
                    $stock_detail_a->setAdjustmentType($type[$index]);
                    $stock_detail_a->setTemporary(0);
                    $stock_detail_a->setIsReceived($detail_id);
                    $stock_detail_a->setVvmStage($vvmstage);
                    $this->_em->persist($stock_detail_a);
                    $this->_em->flush();
                }
    ...
    public function stockGatepassSearch($date_from, $dateto) {
        ...
        if (!empty($date_from) && !empty($dateto)) {
            $where[] = "sm.transactionDate BETWEEN '" . $date_from . "' AND '" . $dateto . "'";
        }
        if (!empty($wh_id)) {
...
//echo $str_sql->getQuery()->getSql();
    ...
    public function uploadReceivedQuantityViaScanner() {
        ...
        $stock_receive->setVvmStage($vvmstage);
...
//        $wh_id = $this->_identity->getWarehouseId();
	...
	abs(GetPicked(stock_detail.pk_id)) AS place_quantity
        ...
        INNER JOIN item_pack_sizes ON stock_batch.item_pack_size_id = item_pack_sizes.pk_id
        WHERE
        stock_master.transaction_type_id NOT IN (" . Model_TransactionTypes::TRANSACTION_RECIEVE . "," . Model_TransactionTypes::LOST_RECOVERED . "," . Model_TransactionTypes::PHYSICALLY_FOUND . "," . Model_TransactionTypes::PHYSICALLY_NOT_FOUND . ")
...
//	) > 0";
    ...
    public function getDataEntryStatusFederal() {
        ...
        if ($this->form_values['role_id'] == 3 || $this->form_values['wh_type'] == 1 || $this->form_values['role_id'] == 4 || $this->form_values['role_id'] == 6 || $this->form_values['role_id'] == 7 || $this->form_values['role_id'] == 17) {
            $str_qry1 = "SELECT
                ...
                from_warehouses.pk_id as wh_from_id
                FROM
                stock_detail
                ...
                WHERE
                stakeholders.pk_id = 1 AND
                stakeholders.geo_level_id = 1 AND
                stock_master.draft = 0 AND
                stakeholders.stakeholder_type_id = 1 AND
                warehouses.status = 1
                ORDER BY
                transactionDate DESC LIMIT 1";
    ...
    public function getProductLedger() {
        ...
        $str_sql = $this->_em->createQueryBuilder()
                ...
                ->andWhere("(sb.warehouse = " . $warehouse . " AND sd.adjustmentType >= 2) OR (sb.warehouse = " . $warehouse . " AND sd.adjustmentType = 1 )")
                ->andWhere("DATE_FORMAT(sm.transactionDate,'%Y-%m-%d') BETWEEN '" . App_Controller_Functions::dateTimeToDbFormat($form_values['from_date']) . "' AND '" . App_Controller_Functions::dateTimeToDbFormat($form_values['to_date']) . "'")
    ...
    public function getProductOB() {
        ...
        $str_sql = $this->_em->createQueryBuilder()
                ...
                ->andWhere("DATE_FORMAT(sm.transactionDate,'%Y-%m-%d') < '" . App_Controller_Functions::dateTimeToDbFormat("01/01/2013") . "'")
...
// For Previous Date
    ...
    public function getSufficientProductReport() {
        ...
        $str_qry = "SELECT
	ROUND(ABS(A.issue)/12,2) as avg_issue,
	A.item_name,
	B.soh,
        A.amc
FROM
...
//echo $str_qry;
    ...
    public function priorityVaccinesDistribution() {
        ...
        $str_qry = "SELECT
                    ...
                    WHERE
                    stock_batch.warehouse_id = 159 AND
                    item_pack_sizes.item_category_id = 1 
                    $where
                    GROUP BY
